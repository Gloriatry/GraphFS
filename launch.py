"""Launching tool for DGS distributed training"""
import os
import stat
import sys
import subprocess
...
def kill_process(ip, port, pids):
    ...
    for pid in pids:
        ...
        kill_cmd = 'ssh -o StrictHostKeyChecking=no -p ' + str(
            port) + ' ' + ip + ' \'kill {}\''.format(pid)
...
def get_killed_pids(ip, port, killed_pids):
    ...
    ps_cmd = 'ssh -o StrictHostKeyChecking=no -p ' + str(
        port) + ' ' + ip + ' \'ps -p {} -h\''.format(killed_pids)
...
def execute_remote(cmd, ip, port, thread_list, id, log_dir, is_server=False):
    # prepare log dir
    if not os.path.exists(log_dir):
        os.mkdir(log_dir)
    stdout_file = log_dir + '/' + str(id) + '.out'
    stderr_file = log_dir + '/' + str(id) + '.err'

    if is_server:
        stdout_file = log_dir + '/' + 'server-' + str(id) + '.out'
        stderr_file = log_dir + '/' + 'server-' + str(id) + '.err'
    """execute command line on remote machine via ssh"""
    cmd = 'ssh -o StrictHostKeyChecking=no -p ' + str(
        port) + ' ' + ip + ' \'' + cmd + '\'' \
...
def get_remote_pids(ip, port, cmd_regex):
    ...
    ps_cmd = 'ssh -o StrictHostKeyChecking=no -p ' + str(
        port
    ) + ' ' + ip + ' \'ps -aux | grep python | grep -v StrictHostKeyChecking\''
    res = subprocess.run(ps_cmd, shell=True, stdout=subprocess.PIPE)
    for p in res.stdout.decode('utf-8').split('\n'):
        l = p.split()
        if len(l) < 2:
    ...
    ps_cmd = 'ssh -o StrictHostKeyChecking=no -p ' + str(
        port) + ' ' + ip + ' \'pgrep -P {}\''.format(pid_str)
    res = subprocess.run(ps_cmd, shell=True, stdout=subprocess.PIPE)
...
def submit_jobs(args, udf_command):
    ...
    server_cmd = server_cmd + ' ' + 'OMP_NUM_THREADS=' + str(
    ...
    server_cmd = server_cmd + ' ' + 'DGL_GRAPH_FORMAT=' + str(
        args.graph_format)
    for i in range(len(hosts) * server_count_per_machine):
        ip, _ = hosts[int(i / server_count_per_machine)]
        cmd = server_cmd + ' ' + 'DGL_SERVER_ID=' + str(i)
        cmd = cmd + ' ' + udf_command + ' --id ' + str(i)
    ...
    # launch client tasks
    client_cmd = 'DGL_DIST_MODE="distributed" DGL_ROLE=client DGL_NUM_SAMPLER=' + str(
        args.num_samplers)
    ...
    client_cmd = client_cmd + ' ' + 'DGS_INTERFACE=' + str(args.interface)
    if os.environ.get('OMP_NUM_THREADS') is not None:
        client_cmd = client_cmd + ' ' + 'OMP_NUM_THREADS=' + os.environ.get(
            'OMP_NUM_THREADS')
    else:
        client_cmd = client_cmd + ' ' + 'OMP_NUM_THREADS=' + str(
            args.num_omp_threads)
    if os.environ.get('PYTHONPATH') is not None:
        client_cmd = client_cmd + ' ' + 'PYTHONPATH=' + os.environ.get(
            'PYTHONPATH')
    client_cmd = client_cmd + ' ' + 'DGL_GRAPH_FORMAT=' + str(
        args.graph_format)

    torch_cmd = '-m torch.distributed.run'
    ...
    for node_id, host in enumerate(hosts):
        ip, _ = host
        new_torch_cmd = torch_cmd.replace('node_rank=0',
                                          'node_rank=' + str(node_id))
        if 'python3' in udf_command:
            new_udf_command = udf_command.replace('python3',
                                                  'python3 ' + new_torch_cmd)
        elif 'python2' in udf_command:
            new_udf_command = udf_command.replace('python2',
                                                  'python2 ' + new_torch_cmd)
        else:
            new_udf_command = udf_command.replace('python',
                                                  'python ' + new_torch_cmd)
        cmd = client_cmd + ' ' + new_udf_command + ' --id ' + str(node_id)
        cmd = 'cd ' + str(args.workspace) + '; ' + cmd
        execute_remote(cmd, ip, args.ssh_port, thread_list, node_id,
                       args.log_dir)
...
def main():
    ...
    parser.add_argument('--num_trainers',
                        type=int,
                        help='The number of trainer processes per machine')
    parser.add_argument('--num_omp_threads',
                        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        default='dgslog',
        help='The directory contains all logs of remote processes.')
    ...
    if args.num_omp_threads is None:
        ...
        print('The number of OMP threads per trainer is set to',
              args.num_omp_threads)
    else:
        print('The number of OMP threads per trainer is set to',
              args.num_omp_threads)
